$(function() {
    ...
    function errorNotification(text) {
        notifList.append(`<li>${SVGS.error}<span>${text}</span><button></button></li>`); // Está sin probar ni estilizar.
    }

    function successfulNotification(text) {
        notifList.append(`<li>${SVGS.successful}<span>${text}</span><button></button></li>`); // Está sin probar ni estilizar.
    }
})